...
class VehiclePIDController(object):  # pylint: disable=too-few-public-methods
    ...
    def run_step(self, target_speed, current_speed, current_pose, waypoint):
        ...
        control.steer = -steering#这里这个负号我没太看明白
...
class PIDLongitudinalController(object):  # pylint: disable=too-few-public-methods
    ...
    def run_step(self, target_speed, current_speed):
        ...
        rclpy.logging.get_logger("纵向PID").info("纵向PID参数({:.3f},{:.3f},{:.3f}),各项误差({:.3f},{:.3f},{:.3f}),期望速度{:.3f},实际速度{:.3f},控制量{:.3f}".format(self._K_P,self._K_I,self._K_D,
                                                                                self.error,self.error_integral,self.error_derivative,target_speed,current_speed,np.clip(output, -1.0, 1.0)))
        return np.clip(output, 0.0, 1.0)


class PIDLateralController(object):  # pylint: disable=too-few-public-methods
    #为啥要声明成类，而不是写一个函数，主要是想储存那些误差变量。
    """
    ...
    def run_step(self, current_pose, waypoint):
        ...
        v_vec = np.array([v_end.x - v_begin.x, v_end.y - v_begin.y, 0.0])#自车方位向量
        w_vec = np.array([waypoint.position.x -
                          v_begin.x, waypoint.position.y -
                          v_begin.y, 0.0])#自车到目标点的向量
        ...
        self.error_derivative = self.error - previous_error
        output = self._K_P * self.error + self._K_I * self.error_integral + self._K_D * self.error_derivative
        rclpy.logging.get_logger("横向PID").info("横向PID参数({:.3f},{:.3f},{:.3f}),各项误差({:.3f},{:.3f},{:.3f}),控制量{:.3f}".format(self._K_P,self._K_I,self._K_D,self.error,self.error_integral,self.error_derivative,np.clip(output, -1.0, 1.0)))
