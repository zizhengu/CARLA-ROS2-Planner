...
class LocalPlanner(CompatibleNode):
    ...
    def run_step(self):
        ...
        with self.data_lock:
            ...
            self.loginfo("当前位置({},{})".format(self._current_pose.position.x,self._current_pose.position.y))

            self.loginfo("目标位置({},{})".format(target_pose.position.x,target_pose.position.y))
